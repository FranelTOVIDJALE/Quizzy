...
const Quiz = () => {
  ...
  const onAnswerSelected = (answer, index) => {
    ...
    if ( countP <= 1 &&  answer === correctAnswer && questions[0] ) {
      ...
      document.getElementById(answer).setAttribute('style','background-color:red;color:white;')
          ...
          /******** stop propagation**********/     
         const answerElement = document.getElementById(Trueresult);
...
}else if (activeQuestion === questions.length - 3 && x === 1 && answer !== correctAnswer ) {
...
} else if (activeQuestion === questions.length - 2 && x === 2 && answer !== correctAnswer ) {
...
} else if (activeQuestion === questions.length - 1 && x === 3 && answer !== correctAnswer ) {
  ...
  return (
    ...
    <img style={{margin:'auto'}} alt='logo' src='https://raw.githubusercontent.com/FranelTOVIDJALE/reactImgs/8d4f0f2d3975634457a49fc9a35469cc4a56c420/SuccesImage.svg' />
         ...
         ><a style={{textDecorationLine:'none'}} href="https://quizmatic.surge.sh/"> Try again</a></button>
        </div>
      )}
    </div>
  )
